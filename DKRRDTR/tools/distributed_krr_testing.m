function [pi_action, time_cost] = distributed_krr_testing(algo, X, action_mode, Nr)
...
%             tic;
%             for j = 1:size_of_action_space        
%                 qi_results_all(:,j,k) = krr_testing(algos(k,j), X);
%             end
%             ttmp = toc;
%             time_local(k) = ttmp;
%         end
...
%             tic;
%             for j = 1:size_of_action_space        
%                 qi_results_all(:,j,k) = krr_testing(algos(k), X(:,:,j));
%             end
%             ttmp = toc;
%             time_local(k) = ttmp;
%         end
