function [pi_action, time_cost] = distributed_ls_testing(algo, X, action_mode, Nr)
    ...
    if strcmp(action_mode, 'separate')
        for k = 1:num_machines
            ...
            for j = 1:size_of_action_space        
                qi_results_tmp(:,j) = lsw_testing(algos(k,j), X);
            end
            ttmp = toc;
            time_local(k) = ttmp;
        ...
        for k = 1:num_machines
            ...
            tic;
            for j = 1:size_of_action_space        
                qi_results_tmp(:,j) = lsw_testing(algos(k), X(:,:,j));
            end
            ttmp = toc;
            time_local(k) = ttmp;
...
%             tic;
%             for j = 1:size_of_action_space        
%                 qi_results_all(:,j,k) = lsw_testing(algos(k,j), X);
%             end
%             ttmp = toc;
%             time_local(k) = ttmp;
%         end
...
%             tic;
%             for j = 1:size_of_action_space        
%                 qi_results_all(:,j,k) = lsw_testing(algos(k), X(:,:,j));
%             end
%             ttmp = toc;
%             time_local(k) = ttmp;
%         end
