function [qfunctions, ret_maxQi_plus_one, time_cost] = cancer_vfunction_training(qt, opt)
...
for i = trajectory_length:-1:1
    ...
    if i == trajectory_length
        [maxQi_plus_one, qfi, time_cost(i)] = cancer_qfunction_training(qfi, qtdata{i});
    else
        qti_i_plus_one = {qtdata{i}, maxQi_plus_one};
        [maxQi_plus_one, qfi, time_cost(i)] = cancer_qfunction_training(qfi, qti_i_plus_one);
    end
    qfunctions{i} = qfi;
