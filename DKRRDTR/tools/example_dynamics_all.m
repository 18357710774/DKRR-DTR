function [TotalTime, NumActions, InitialWellness] = example_dynamics_all(e_test, num_stages)
...
for i = 1:num_stages
    ...
    for j = 1:size_of_action_space^(i-1)   % j = 1,2,4
        for k = 1:size_of_action_space
            Wi_tmp = max(Wi(:,j)-e_test.actions(k,1), 0);
            W_action{i} = [W_action{i}, Wi_tmp];
            Mi_tmp = min(e_test.actions(k,2)./Wi(:,j), 1);
            M_action{i} = [M_action{i}, Mi_tmp];
        end
    end
    
    Ri_tmp = (1/e_test.Mdot)*(1-M_action{i})./M_action{i};
    Ri_tmp(AtRisk{i}==0) = 0;
    R{i} = Ri_tmp;
    ...
    W{i+1} = W_action{i} + (1-W_action{i}).*(1-2.^(-e_test.Wdot*U{i}));
    W{i+1}(AtRisk{i}==0) = 0;

    M{i+1} = M_action{i} + e_test.Mdot*M_action{i}.*U{i};
    M{i+1}(AtRisk{i}==0) = 0;
end
