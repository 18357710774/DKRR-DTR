function [CSP_mean, time_cost, W_mean, M_mean, WM_mean, R_mean, Death_mean, Cured_mean, AtRisk_mean, pi_actions, bb] ...
            = distributed_cancer_example_dynamics_allstages(vf, a, opt)
valgo = vf.qfunctions;
Nr = vf.info_split.Nr;
...
time_cost = struct('time_local', {}, 'time_synthesize',{}, 'time_state_compute',{});
% Test
for i = 1:num_stages
    action_space = valgo{i}.action_space;
    X = set_cancer_qtrajectory_test_data(W, M, pi_actions, i, action_space, fea_mode, action_mode);

    [pi_actions(:, i), time_cost_tmp] = distributed_cancer_qfunction_testing(valgo{i}, X, Nr);
    time_cost(i).time_local = time_cost_tmp.time_local;
    time_cost(i).time_synthesize = time_cost_tmp.time_synthesize;
