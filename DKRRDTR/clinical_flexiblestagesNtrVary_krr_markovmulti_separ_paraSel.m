% parameter selection step for krr (multiple + separate case)
...
n_train_seq = ceil(10000./(5:5:500));    % the number of training samples
...
m_mean_time_hat_opt_all = zeros(1, length(n_train_seq));
...
for n_train = n_train_seq
    ...
    savefile = [cd '\SynResults\clinical_flexiblestagesNtr' num2str(n_train) ...
                '_' opt.algoname  '_markovmulti_separ' '_paraSel.mat'];
    ...
    for Ex = 1:ExNum
        ...
        for k = 1:size(para,2)
            ...
            disp(['ntr#' num2str(n_train)  '   Ex#' num2str(Ex)  '  sigma=' num2str(para(1,k)) ...
                  '  lambda='   num2str(para(2,k))  '  mean_time_hat=' ...
                  num2str(mean_time_hat(Ex, k)) '  time_cost=' num2str(t) 'seconds']);
        end
    end
    ...
    m_mean_time_hat_opt_all(kk) = m_mean_time_hat_opt;

    save(savefile, 'example', 'n_train', 'n_test', 'ExNum','opt', 'sigma_seq', ...
                   'lambda_seq', 'para', 'mean_time_hat', 'm_mean_time_hat', ...
                   'lambda_opt', 'sigma_opt', 'm_mean_time_hat_opt');
    clear lambda_seq para mean_time_hat m_mean_time_hat ...
          lambda_opt sigma_opt m_mean_time_hat_opt
end

savefile = [cd '\SynResults\clinical_flexiblestagesNtrVary_' ...
            opt.algoname  '_markovmulti_separ_paraSel.mat'];

save(savefile, 'example', 'n_train_seq', 'n_test', 'ExNum', 'opt', ...
               'lambda_opt_all', 'sigma_opt_all', 'm_mean_time_hat_opt_all');