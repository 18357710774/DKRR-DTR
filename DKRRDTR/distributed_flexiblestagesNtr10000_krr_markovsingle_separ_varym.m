% DKRR-DTR with the selected parameters  (single + separate case)
...
opt.algoparams(1) = 0.0127;         % KRR: the fisrt parameter is width of kernel, 
opt.algoparams(2) = 0.0156;         %      the second parameter is regularization

if ~exist([cd '\SynResults'], 'dir')
	mkdir([cd '\SynResults']);
end
savefile = [cd '\SynResults\distributed_clinical_flexstageNtr' num2str(n_train) ...
            '_' opt.algoname  '_markovsingle_separ_varym_Usplit.mat'];
