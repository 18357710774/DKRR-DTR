% DKRR-DTR with the selected parameters  (non-markov + joint case)
...
opt.algoparams(1) = 0.0546;         % KRR: the fisrt parameter is width of kernel, 
opt.algoparams(2) = 0.0078;         %      the second parameter is regularization

if ~exist([cd '\SynResults'], 'dir')
	mkdir([cd '\SynResults']);
end
savefile = [cd '\SynResults\distributed_clinical_flexstageNtr' num2str(n_train) ...
            '_' opt.algoname  '_markovnon_mix_varym_Usplit.mat'];
